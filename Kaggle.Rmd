...
```{r}
y1 = train$mean_grade
x = data.matrix(train %>% select(famrel))
model = glm(formula = y1 ~ x, family = binomial(link = logit))
summary(model)
```
...
#plot(x,y)
o = order(x)
logit_ypredit=0.19415*x+0.03977
y2=exp(logit_ypredit)/(1+ exp(logit_ypredit)) # transfo inverse de logit
ggplot(data = data.frame(x = x, y = y1, y2 = y2), aes(x = factor(x))) +
...
```{r}
x <- data.matrix(train %>% select(address,famsize,Pstatus,Medu,Fedu,Mjob,Fjob,reason,
                              guardian,traveltime,famsup,nursery,famrel))
y <- train$mean_grade
model <- glm(formula = y ~ x, family = binomial(link = logit))
summary(model)
```
```{r}
