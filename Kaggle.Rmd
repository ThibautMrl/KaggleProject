...
```
```{r}
# transformer toutes les variables catégorielles en variables binaires
# utiliser la fonction model.matrix pour convertir les variables catégorielles en variables binaires
dummies <- model.matrix(~ . - 1, data = train)

# renommer les variables binaires
colnames(dummies) <- paste(colnames(dummies), sep = "_")

# joindre les variables binaires à la dataframe d'origine
df <- cbind(train, dummies)
```


```{r}
xtrain <- train %>% select(address,famsize,Pstatus,Medu,Fedu,Mjob,Fjob,reason,
...
model <- glm(formula = y ~ .,data = xtrain, family = binomial(link = logit))
summary(model)
```
...
#pred
pred <- model %>% predict(xtest, type = "response")
pred
```
